...
description: Agent skills for working with Minions Test-reports MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-test-reports data.
---

# Minions Test-reports Agent Skills

Skills for agents operating on the `minions-test-reports` toolbox.
...
```bash
# TypeScript
pnpm add @minions-test-reports/sdk
...
The `test-reports` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-test-reports/sdk/schemas';
...
```python
from minions_test_reports.schemas import custom_types
...
- This agent only writes to `minions-test-reports` — it reads from other toolboxes but never writes to them
